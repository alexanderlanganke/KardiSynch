...
async function runTest() {
    ...
    mockParser.parseFile.mockImplementation(async (file: string) => {
        if (file === structuredFile) {
            return {
                ...
                patient: { last_name: 'Doe', first_name: 'John', dob: '1980-01-01', id: 'P-1' },
        ...
        if (file === associatedPdf) {
            return {
                manufacturer: 'Medtronic',
                interrogation_date: '2025-11-28T10:00:00',
                patient: { last_name: 'Doe', first_name: 'John', dob: '1980-01-01' },
        ...
        if (file === standalonePdf) {
            return {
                ...
                patient: { last_name: 'Smith', first_name: 'Jane', dob: '1990-05-05' },
    ...
    // Mock DB Responses
    mockDb.storeReport.mockResolvedValue({ reportId: 'R-1', patient: { id: 'P-1', last_name: 'Doe' } });
    mockDb.findPatient.mockImplementation(async (last: string, dob: string) => {
        if (last === 'Smith') return { id: 'P-2', last_name: 'Smith', first_name: 'Jane' };
        return null;
    });
    mockDb.findPatientBySerial.mockImplementation(async (serial: string) => {
        if (serial === '888888') return { id: 'P-3', last_name: 'Brown', first_name: 'Bob' };
        return null;
    });
