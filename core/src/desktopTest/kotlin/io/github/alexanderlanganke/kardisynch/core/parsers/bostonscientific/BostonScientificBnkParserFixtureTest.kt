...
@Serializable
private data class FixtureBattery(val remaining_longevity: FixtureMeasurement? = null, val status: String? = null)
...
private data class FixtureLead(
    ...
    val shock_impedance: FixtureMeasurement? = null,
)
...
private data class FixtureReport(
    ...
    val additional_fields: Map<String, JsonElement>? = null,
)
...
/**
 * Cross-validates the ported [parseBostonScientificBnk] against the
 * ORIGINAL TypeScript parser's real output for every real .bnk sample in
 * test/boston bnk (14 files) — see
 ...
 */
class BostonScientificBnkParserFixtureTest {
    private val fixtures: Map<String, FixtureReport> by lazy {
        val json = Json { ignoreUnknownKeys = true }
        val text = requireNotNull(
            javaClass.classLoader.getResourceAsStream("boston-fixtures.json"),
        ) { "boston-fixtures.json not found on the test classpath" }.bufferedReader().readText()
    ...
    fun `matches the reference TypeScript parser on every real boston bnk sample`() {
        ...
        for ((filename, expected) in fixtures) {
            val file = File(bnkDir, filename)
            if (!file.isFile) {
                failures.add("$filename: sample file missing from test/boston bnk/")
                continue
            }
            val actual = parseBostonScientificBnk(file.readText())
            ...
            check(
                ...
                closeEnough(expected.battery.remaining_longevity?.value.asLeadingDouble(), actual.battery.remainingLongevity?.value),
            ...
            for ((i, expectedLead) in expected.leads.withIndex()) {
                ...
                check(
                    "leads[$i].shockImpedance",
                    closeEnough(expectedLead.shock_impedance?.value.asLeadingDouble(), actualLead.shockImpedance?.value),
                    "expected ${expectedLead.shock_impedance?.value}, got ${actualLead.shockImpedance?.value}",
                )
            }

            val expectedAdditional = expected.additional_fields ?: emptyMap()
            check(
                "additionalFields",
                expectedAdditional.keys == actual.additionalFields.keys,
                "expected keys ${expectedAdditional.keys}, got ${actual.additionalFields.keys}",
            )
        }
